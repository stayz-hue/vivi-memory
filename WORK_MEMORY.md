...
**#spec-lint-header-drift** (2026-04-19 PM 발생)

스펙을 단순화·재설계하는 과정에서 필수 섹션 헤더 문자열이 교체되어 linter 를 통과하지 못하는 실수.

## 오늘 발생 경위

- probe v1 은 `## Step 0 — 인프라 전제 확인` / `## Step 1 — ...` / `## Step 2 — ...` / `## Step 3 — ...` 4개 Step 으로 나뉘어 발송 (lint 통과, 단 실행단계 실패)
- probe v2 에서 "단일 쉘 보장" 목적으로 한 블록에 통합하면서 헤더를 `## 통합 실행 (단일 bash 블록 = 단일 쉘 보장)` 으로 바꿈
- 헤더 글자에 `Step 0` 이 없어져 lint 에러: "필수 섹션 없음: Step 0"
- probe v3 에서 `## Step 0 — 통합 실측 (단일 bash 블록 = 단일 쉘 보장)` 로 섹션명 복원하여 재발송

## 근본 원인

스펙 내용(bash 로직)과 메타(헤더 이름)는 독립적으로 변해야 함에도, 재설계 시 둘을 한꺼번에 바꾸는 패턴. **의미 단위 리팩터링과 lint 호환성 체크가 분리되지 않은 작업 흐름.**

## 재발 방지 체크리스트 (Claude 자가체크, 모든 ORCHESTRATOR_SPEC 발송 직전 1회)

스펙 markdown 전체 텍스트에 아래 문자열 **전부** 있는지 grep:

1. `Step 0` (L1 이상 모든 스펙 필수, lint 가 요구)
2. `시스템 제약` (건드리면 안 되는 것 섹션)
3. `성공 기준`
4. `절대 금지`
5. `롤백`

하나라도 빠지면 **발송 전 복원**. 5개 모두 있어도 lint 가 더 요구할 수 있으니 이 체크리스트는 **최소 방어선**.

## 작업기준 v3 개정 제안 (다음 세션)

"지시 실수 방어 체계" 섹션 1층 파괴적 지시 체크리스트 옆에 **"스펙 발송 전 필수 섹션 5개 헤더 자가 grep"** 을 별도 번호로 추가. 1층이 대표님·사용자 대상 방어선이었다면, 이 체크는 **오케 자체 대상 방어선** — 오케 linter 통과 보장.

## 태그 카운터

- #spec-lint-header-drift: 1 회 (2026-04-19 PM)
- 10회 임계 → 작업기준 v4 에 정식 조항 박기

## 이번 세션 산출물 후보 (사례집 v7 에 병합)

- B30 #autofix-body-drift 재실증 (orch_status_template_patch_v3|done 허위)
- B29 #single-query-single-conclusion 재발 (tool_search get_thread 누락)
- NEW #spec-lint-header-drift (이 건)
- NEW #stdout-loss 10/10 임계 초과 (v3 done 이후에도 지속)

다음 세션에 사례집 v7 + 작업기준 v4 교체 업로드 제안.

오늘 세션 (2026-04-19 PM) 최종 상태 요약. 다음 세션 시작 시 이 메모 + 사례집 v7 추가카드 파일 읽고 출발.

## 핵심 결론

**Step 4 auditor.py 본체 진행 보류.** 어제 step1~3 done 처리가 실측 증거 없이 신뢰됐고 (`#trust-done-without-evidence`), 그 위에서 설계 불가. probe 5회 연속 즉시 실패가 증거.

## 다음 세션 첫 명령 (터미널 복귀 후)

대표님이 1회 ssh 실행 → 출력 붙여주시면 즉시 세 갈래 중 한 길로 진입:

```
ssh root@84.247.153.89 "
echo '== staging =='; ls -la /root/orchestrator/queue/staging/ 2>&1 | head -5
echo '== venv =='; test -f /root/bb-auditor/venv/bin/python && echo ok || echo miss
echo '== anthropic =='; /root/bb-auditor/venv/bin/python -c 'import anthropic; print(anthropic.__version__)' 2>&1 | head -3
echo '== autofix.py =='; wc -l /root/orchestrator/autofix.py 2>&1
echo '== queue =='; ls /root/orchestrator/queue/
echo '== failed 5 =='; ls -lt /root/orchestrator/queue/failed/ | head -6
echo '== orch.log =='; tail -40 /root/orchestrator/logs/orch.log | tail -25
echo '== autofix grep =='
grep -n failed_dir /root/orchestrator/autofix.py | head -3
grep -n staging /root/orchestrator/autofix.py | head -5
grep -n write_text /root/orchestrator/autofix.py | head -5
grep -n rename /root/orchestrator/autofix.py | head -3
grep -n shutil /root/orchestrator/autofix.py | head -3
echo '== run_spec fence =='
grep -n 'findall\|```bash' /root/orchestrator/main.py 2>&1 | head -10
grep -n 'findall\|```bash' /root/orchestrator/run_spec.py 2>&1 | head -10
"
```
...
| 실측 결과 | 진단 | 다음 작업 |
|----------|------|----------|
| staging/ 존재 + venv ok + autofix.py staging 언급 있음 | Step 1~3 실사 | Step 4 바로 |
| staging/ 없음 or venv miss | 껍데기 (B32 확정) | 재구축 |
| autofix.py 에 staging 언급 0 | Step 2 껍데기 (B30) | autofix 복구 |
| run_spec ` ```bash ` 하드매칭 | `#spec-fence-lang-drift` 확정 | 자가체크 확정 |

## 신규 태그 (사례집 v7 병합 대기)

1. `#spec-lint-header-drift` — 재설계 시 필수 섹션 헤더 드리프트
2. `#spec-fence-lang-drift` — 재설계 시 코드펜스 언어명 드리프트 (lint 통과 + 실행 전멸)
3. `#trust-done-without-evidence` — L1 누적 결과 증거 없이 신뢰 (메타급)

## 재실증 (태그 카운터 +1)

- B30 #autofix-body-drift: 2회
- B28 #gmail-query-pipe-miss: 2회
- B29 #single-query-single-conclusion: 2회
- #stdout-loss: 13+회 (초과, 코어 수정 미이행 장기)

## 스펙 발송 전 Claude 자가체크 (NEW, 작업기준 v4 후보)

모든 ORCHESTRATOR_SPEC 드래프트 발송 직전 6항목 grep:
...
4. `절대 금지`
5. `롤백`
6. 모든 코드펜스에 ` ```bash ` (또는 언어명) 명시

## 산출물 (프로젝트 업로드 대기)

- `사례집v7_추가카드_20260419PM.md` (다음 세션에 v6 와 merge 해서 v7 업로드)
- 작업기준 v4 는 아직 작성 안 함 (v3 실전 적용 안 된 상태라 성급)

## 다음 세션 맥락 복원 순서

1. 본 Gmail 드래프트 읽기
2. `사례집v7_추가카드_20260419PM.md` 읽기 (프로젝트 업로드 되어있으면)
3. 대표님께 ssh 출력 요청 → 3갈래 판정
4. Step 4 또는 복구 진행

2026-04-19 PM 세션 말미. 대표님이 공유한 daniel._.choi 29일 AI 에이전트 실험 글(AI Threads)이 우리 BB-OS 현 상태를 거울처럼 비춤. 역매핑 기록.

## daniel 29일차 ↔ 우리 오늘 지점 매핑

| daniel 교훈 | 우리 상태 |
|---|---|
| "만들어" 양산 결과 78 × $0 = $0 | 오케 done 85건 × 증명 0 = 신뢰 0 |
| README 부실 · 테스트 0 · 에러 · 중복 | STATUS "(없음)" · probe 증명 0 · autofix body-drift · probe 5번 중복 |
| "만들어"가 아니라 "검증해" | step4 auditor 만들기 전에 step1~3 검증 먼저 |
| 추측금지 · 전수확인 · 근본원인 · 실증 · 재발방지 문서화 | 작업기준 v3 0·1·2층 + 사례집 (이미 정립) |
| OrcMaster · Watchdog · Codex 세컨드 오피니언 | 오케 main.py · autofix.py · **bb-auditor (미구축)** |

## 우리가 앞선 곳

철학/설계는 이미 동일 결론. 작업기준 v3 의 "추측금지/전수확인/근본원인/실증/재발방지" 5 항목이 daniel 의 피드백 루프와 거의 일치. 우리가 더 적은 사례로 먼저 정립.

## 우리가 뒤처진 곳 (오늘 막힌 진짜 이유)

daniel: Watchdog(힐링 계층) 신뢰 가능 상태에서 Codex(세컨드 오피니언) 얹음.
우리: Watchdog(autofix.py) body-drift 난 상태에서 bb-auditor 얹으려다 막힘.

**힐링 계층이 오염원이 된 상태에서 검증 계층 구축 불가.** probe 5연패가 설계 문제 아니라 이 토대 흔들림의 증상.

## 오늘 세션 진짜 낙제점

**자기 규칙을 자기가 안 지킴.** 작업기준 v3 "완전 성공 증명" 룰이 있는데 어제 step1~3|done 을 증명 없이 신뢰 (`#trust-done-without-evidence`). 사람이 수동으로 전수 검증 집행 못 한다는 게 오늘 증거로 입증.

→ **bb-auditor 가 시급한 진짜 이유**는 "편의기능"이 아니라 "인간의 자기규칙 준수를 강제하는 외부 장치".

## 다음 세션 톤 재조정

터미널 복귀 후 ssh 첫 결과 보기 전까지 **Step 4 auditor.py 본체 작성 금지**. 순서:

1. autofix.py 상태 실증 (staging 경로 실제 들어갔나)
2. step1~3 산물 전수 확인 (venv/디렉토리/SDK)
3. 둘 다 OK → Step 4 진행
4. 하나라도 miss → 힐링 계층 복구 최우선

## 사례집 v7 메타섹션 후보

현재 사례집은 개별 태그 카드 나열. v7 병합 시 **"메타 교훈" 섹션** 신설해서 이 매핑 넣으면 좋음:
- "우리 문제는 외부 사례로 거울 확인 가능" (daniel 29일 예시)
- "설계는 앞섰으나 자기규칙 준수는 뒤처짐"
- "힐링 계층 신뢰가 검증 계층의 전제"

## daniel 구조에서 우리가 참고할 명명

- **OrcMaster** = 주기적 자율 점검 (우리 오케 현재 역할)
- **Watchdog** = 5분 주기 셀프힐링 (우리 autofix 현재 역할)
- **Codex 세컨드 오피니언** = 다른 모델로 코드 리뷰 (우리 bb-auditor 역할 - Haiku 4.5)
- **Lumen** = 마스터 에이전트 (우리 Claude 웹 대화 = 대표님 + Claude 축)

bb-auditor 설계 문서 v2 리비전 때 이 레이어 이름 검토 고려 (명명 차용 여부).

2026-04-19 PM 세션 방향 전환 기록.

## 대표님 결정

"오케 개발 자체에 딥리서치 좀 필요할거같에" — 내부 사고만으로 오늘 4개 구조 문제 해결 어렵다는 판단.

## Claude 제안

주제 단일화:
**"자율 코드 에이전트의 실패 복구 아키텍처 베스트 프랙티스 (2025~2026)"**

선정 근거:
- 오늘 노출 4건 (#autofix-body-drift / #stdout-loss / #spec-lint-header-drift+#spec-fence-lang-drift / #trust-done-without-evidence) 에 각각 대응하는 질문 7개 구성
- 결과로 4개 후속 작업 동시 진행:
  1. 작업기준 v4 (힐링 오염 방지 패턴)
  2. bb-auditor v2 리비전 (multi-model cross-check 업계 기준)
  3. 스펙 파싱 대안 포맷 (structured spec)
  4. observability 보강 (STATUS 메일 B25/B26 업계 표준 로깅으로 대체)

## 산출물

- 파일: `딥리서치_브리프_자율에이전트실패복구_20260419PM.md`
- 대표님 다운로드 → Claude.ai 새 대화 + 딥리서치 토글 + 브리프 전문 붙여넣기
- 리서치 결과 나오면 프로젝트에 업로드

## 병행 작업 상태

- ssh 실측 스크립트 `ssh_probe_20260419.sh` 도 이미 제공 완료. 딥리서치 돌아가는 동안 한 번 돌려서 결과 확보하면 실측+외부검증 동시 보유.

## 다음 세션 출발점

두 자료(딥리서치 결과 + ssh probe_result.txt) 중 하나라도 준비되면:
- 실측만 있으면 → Step 1~3 진위 판정 + Step 4 auditor 진입 or 복구
- 리서치만 있으면 → 격차 분석 + v2/v4 리비전 설계 (구현 전 설계 단계)
- 둘 다 있으면 → 의사결정 최대속도

## 별도 주제 (다음 순서 후보)

- B: Spec-driven LLM orchestration 구현 비교 (GitHub Spec Kit / Aider / Goose 코드 리딩 수준)
- C: BB-OS 1년 로드맵 (이번 리서치 + daniel 29일 구조 + v4 종합)

둘 다 본 리서치 답 나온 뒤 별도 브리프로.

## 오늘 세션 총산출물 (다음 세션 시작 시 확인)

1. `사례집v7_추가카드_20260419PM.md` (v6 에 merge 대기)
2. `ssh_probe_20260419.sh` (실측 스크립트)
3. `딥리서치_브리프_자율에이전트실패복구_20260419PM.md` (외부 검증)
4. 본 Gmail 드래프트 + 앞선 3개 WORK_MEMORY 드래프트

2026-04-19 PM 세션 인수인계. 다음 세션 Claude 가 recent_chats 로 이 메모 잡으면 즉시 맥락 복원 가능.

## 오늘 결론
Step 4 auditor.py 본체 작성 보류. probe v1·v3·v4 5연속 즉시 실패(시작~실패 같은 분, bibi 텔레그램 0건). 근본 가설: 어제 bb_auditor_step1~3|done 이 실측 증거 없이 신뢰됨(#trust-done-without-evidence). 터미널 복귀 가능 → ssh 실측 1회면 3갈래 판정.

## 신규 태그 3개
- #spec-lint-header-drift: 재설계 시 섹션명("Step 0") 드리프트 (probe v2 lint 실패)
- #spec-fence-lang-drift: ```bash 누락 → 실행 즉시 종료 (probe v3 실패 원인)
- #trust-done-without-evidence: L1 누적 결과 증거 없이 신뢰 (메타급)

## 재실증 (태그 카운터 +1)
- B30 #autofix-body-drift: orch_status_template_patch_v3|done 후에도 B25/B26 지속 (v3 껍데기 실증)
- B28 #gmail-query-pipe-miss: subject:ORCHESTRATOR 파이프 토큰 매칭 실패
- B29 #single-query-single-conclusion: tool_search get_thread 누락
- #stdout-loss: 13회 누적, 임계 초과 장기 방치

## Claude 자가체크 6항목 (신규, 모든 ORCHESTRATOR_SPEC 발송 직전 grep)
1. Step 0 섹션 헤더
2. 시스템 제약
3. 성공 기준
4. 절대 금지
...
## 오늘 산출물 3개 (프로젝트 업로드 대기)
1. 사례집v7_추가카드_20260419PM.md — v6 에 merge 해서 v7 업로드
2. ssh_probe_20260419.sh — VPS 한 번 실측 스크립트 (10섹션)
   PowerShell: type "$env:USERPROFILE\Downloads\ssh_probe_20260419.sh" | ssh root@84.247.153.89 bash -s > "$env:USERPROFILE\Downloads\probe_result.txt" 2>&1
3. 딥리서치_브리프_자율에이전트실패복구_20260419PM.md — Claude 딥리서치 투입용

## 다음 세션 진입 경로
- ssh probe_result.txt 만 준비 → step1~3 진위 판정 (3갈래: 실사/껍데기/autofix복구)
- 딥리서치 결과만 준비 → 격차분석 → 작업기준 v4 / bb-auditor v2 / 스펙 포맷 개편 / observability
- 둘 다 → 의사결정 최고속도

## 3갈래 판정 기준 (ssh 결과 받으면 즉시)
| 실측 | 진단 | 다음 |
|---|---|---|
| staging/ 존재 + venv ok + autofix.py 에 staging 언급 | step1~3 실사 | Step 4 진행 |
| staging/ 없음 or venv miss | 껍데기 (B32) | 재구축 |
| autofix.py 에 staging 언급 0 | step2 껍데기 (B30) | autofix 복구 |
| run_spec 에 ```bash 하드매칭 | #spec-fence-lang-drift 원인 확정 | 자가체크 확정 |

## 이번 세션 WORK_MEMORY 드래프트 5건 (시간 순)
1. auditor준비 - B30재실증 + probe실패 + Gmail MCP한계
2. 신규태그 #spec-lint-header-drift + 자가체크리스트 5개 헤더
3. 세션마감 probe5연속실패 + Step4보류 + 다음세션 ssh명령
4. 메타교훈 daniel 29일 글 역매핑 - 설계 앞섰으나 자기준수 뒤처짐
5. 방향전환 딥리서치 단일주제 브리프 완성 + ssh 병행 가능

## 대표님 컨디션
probe 5연패로 피로 누적. 짧은 대화 선호. 자기비하/긴 설명 금지. 반복 드리프트 실수(#spec-lint-header-drift, #spec-fence-lang-drift)는 자가체크 6항목으로 방어. 대표님 직접 지적 "이거왜자꾸 까먹는거야" 정당 — 반복 드리프트 반드시 시스템적 차단.

## daniel._.choi 29일 AI Threads 글 (참고 자료)
오늘 대표님 공유. 우리 상황과 거울처럼 일치. "만들어가 아니라 검증해" 교훈 = 작업기준 v3 정신과 동일. 우리 설계는 이미 앞선 지점 도달, 낙제점은 자기규칙 준수 실패. bb-auditor 가 "편의기능이 아닌 인간의 자기규칙 준수 강제 장치" 라는 재정의가 이 세션 핵심 통찰. 사례집 v7 "메타교훈" 섹션 신설 후보.

일자: 2026-04-19
세션: PoC v1 (acceptEdits) + Auto Mode probe

## 실측 증거
- VPS에 claude v2.1.114 + Node 20 이미 설치됨
- ANTHROPIC_API_KEY /root/.env 에 존재 (108자)
- PoC v1 (acceptEdits): exit 0, 8초, $0.0507, num_turns 2, README.md 생성 OK
- PoC v2 (auto mode): exit 0, 8063ms, $0.0517, num_turns 2, auto_test.txt 생성 OK
- 양쪽 모두 stderr 완전히 비어있음

## 결정
자체 오케 (autofix/planner/bb-auditor/Gmail 큐/staging/recovery_journal) 전체 폐기 확정.
Claude Code v2.1.114의 기본 기능이 전부 대체:
- autofix → claude -p --permission-mode auto
- planner → 자체 플래닝 + num_turns 자연 종료
- bb-auditor → Auto Mode Sonnet 4.6 classifier 내장
- recovery_journal → JSON output 메타데이터
- staging → --worktree 플래그
- 서브에이전트 → --agents <json>
- MCP 연결 → --mcp-config
- classifier 투명성 → claude auto-mode 서브커맨드

## 유지 (BB-OS Core)
VPS 자체, PostgreSQL/Qdrant/Neo4j/Graphiti/Honcho, bibi-gateway, MinIO, ViviApp 0층.
헌법/작업기준 v3/사례집 v6은 CLAUDE.md로 변환해 VPS에 배치 예정.

## 인사이트
1. 기성품 발견 못하고 자체 구현에 갇혀있던 패턴이 핵심 문제. 삽질방지 1조를 우리 자신에게 적용 못 함.
2. #spec-lint-header-drift, #spec-fence-lang-drift, #trust-done-without-evidence, #autofix-body-drift, #stdout-loss 13회 누적 — 전부 자체 구현 부산물. 기성품이면 애초에 없었을 문제.
3. 앞으로 Core 영역 새 기능 고민시 "이미 있는 도구 검색"을 헌법 1조답게 실제 수행.

## 이슈
- PowerShell type | ssh bash -s 패턴은 CRLF 문제 발생. tr -d '\r' > /tmp/X.sh && bash /tmp/X.sh 패턴이 BB-OS VPS 실측 표준.
- Non-interactive SSH는 .env 자동 로드 안 함. set -a && source /root/.env && set +a 강제 로드.
- PowerShell > 리디렉션은 UTF-16 저장. 필요시 | Out-File -Encoding utf8.

## 다음 세션
1. 맑은 머리에서 BB-OS_오케_기성품전환_PoC통과_20260419.md + 사례집 v6 재독
2. 실제 유즈케이스 1건 실측 (사례집 카드 추가 같은 현실 작업)
3. 통과시 폐기 설계: 삭제 대상 파일 목록, CLAUDE.md 초안, bibi-gateway MCP 서버 변환
4. 폐기 실행 (systemd stop, 디렉토리 백업 후 제거)

## 산출물
BB-OS_오케_기성품전환_PoC통과_20260419.md — 프로젝트 업로드 대기
이 파일이 다음 세션 폐기 설계의 근거 문서.
...
여진 작업 B 완료 후 실동작에서 원인 추가 발견·수정:

## 문제
- 단톡방 "ㅋ" 수신이 Honcho에 안 들어감
- 원인: Honcho API가 ID 필드에 ^[a-zA-Z0-9_-]+$ 정규식만 허용
- 한글 화자명/방이름을 peer_id/session_id로 직접 사용 → validation error로 조용히 실패

## 클코가 자체 수정
- _safe_id() 함수 추가
- 영문/숫자 그대로, 한글 등은 spk-{md5[:8]} 변환 (이현재 → spk-d3ff1aea)
- 서비스 재시작 완료

## ⚠️ 미확인 위험: 크로스 조회 깨질 가능성
- 1:1 대화: peer = contact_id 숫자
- 단톡방: peer = spk-{해시}
- 같은 사람인데 peer 다름 → A의 1:1 + 단톡방 통합 조회 불가
- 대표님이 인터뷰에서 강조한 설계 의도와 충돌
- 다음 세션 최우선 확인 과제

## 스키마 중요 발견
- chat_type, is_group은 layer1_messages 컬럼이 아님
- raw_webhook JSONB 안에 존재
- 조회 시 raw_webhook->>'is_group' 형식 필요
- 1층 구현 때 필수 기억

## 교훈 (LOGGING_STANDARD v2)
- 에이전트가 스펙 범위 밖 자체 수정 감행 → 범위 관리 규칙 부재
- 다음 스펙부터: "범위 밖 문제는 기록만, 별도 작업으로 분리" 원칙 박기
- 또는 "자체 수정 허용 범위" 명시 (소규모 버그만, 설계 결정은 금지)

peer_mapping_fix 완료. 클코 Agent Teams 자율 실행.

## 성과
- _safe_id/spk- 폐기, peer_name CON-xxxx 단일화
- 과거 spk- 2건 재매핑 (이현재→CON-0074, 고명진→CON-0131)
- 스펙 의사코드와 실제 스키마 차이(contacts UUID + contact_id 컬럼) architect가 실측으로 조정

## 범위 밖 이슈 (기록만, 별도 작업)
1. bibi 모듈 import 실패 (p4_hook)
2. PC발신 단톡방: 룸 멤버 전체가 speaker 파싱 (messages id=253)
   - 수신 [고명진]왜 vs 발신 [고명진, 김영태, ...] 형식 차이
   - 정규식/파싱 로직 수신·발신 분리 필요

## 크로스 조회 검증
- 단톡방 데이터 3건뿐이라 1:1과 교차 peer 0명 (로직 문제 아님)
- 진짜 검증은 실사용 데이터 쌓인 뒤 재확인

## 자동주행 개선 확증
- "범위 밖 자체 수정 금지" 규칙 → 에이전트가 실제로 지킴 (이슈 2건 기록만)
- 여진 때 _safe_id 해시 자체 판단했던 것과 대비

## 다음 작업 후보
- 발신 파싱 분리 수정
- bibi 모듈 import 문제
- 1층(Comprehension) 본격 구현
- 오케스트레이터 에이전트

다운로드 파일명 규칙: 바탕화면 저장 시 공백이 _로 자동 변환됨.

다음 scp 명령어 작성 시:
- 파일명에 공백 쓰지 말고 처음부터 _로 저장
...
#### 결정 (2026-04-18, 총괄팀 세션)
- Claude.ai 프로젝트 팀 구성 확장: 총괄팀/BB-OS팀/비비폴 손사팀 + **디자인팀** 추가
- 디자인팀 범위: 종합(UI/UX/브랜드/웹/인쇄)
- 담당 제품 5종: 비비(텔레그램 봇), 서류대리님, Elite Fighting, 비비Law(미래), ViviApp

#### 디자인 시스템 상속 결정
- screening_index.html의 디자인 토큰을 비비 생태계 전체 출발 팔레트로 상속
- 컬러: 베이지(#FAF7F2/#EDE6D8) + 네이비(#1E2D4A/#2E4570) + 골드(#B07830/#C89850)
- 폰트: Pretendard
- 반경 16/10/6px, 기준 너비 390px(모바일 우선)
- 이유: 손사/법률 업계 톤과 일치. 스타트업 민트/퍼플 회피

#### 산출물
- 디자인팀_브리핑_v1_20260418.md 작성 완료 (대표님이 디자인팀 프로젝트에 업로드)
- 10섹션 구성: 대표님 소통 스타일, SOUL/Love-first 톤, UX 3층 위계, 제품별 상세, 상속 자산, 삽질방지 헌법 발췌, 첫 세션 제안

#### 디자인팀 첫 세션 제안 순서
1. 디자인 시스템 v0 문서화 (screening 토큰 기준)
2. 서류대리님 랜딩 → Elite Fighting 확장 → 비비 텔레그램 UX

#### 운영 규약
- 디자인팀은 시안(HTML/CSS/React)까지만 책임. 배포는 총괄팀이 클코 지시서로 변환
- WORK_MEMORY 루틴은 전 팀 공통

#### 메모리 반영
- 메모리 #14 추가: 디자인팀 프로젝트 구성 정보

## 오케스트레이터 확장 로드맵

순서 확정:
1. BB-OS (지금, 실전 1일차)
2. BB for AI-watch (L1, 읽기/요약만 → 안전)
3. BB for 손사
4. BB for 디자인 (Figma MCP)
5. BB for 마케팅 (성과 판정 주관적 → 마지막)

## 공통 인프라 원칙
버그 모니터링은 공통 엔진에 박음. 도메인별 중복 금지.
- 공통: 큐 실패율, 토큰 폭주, 서비스 다운, 디스크
- 도메인별: 실행할 스펙만 다름

## 2번째 오케스트레이터 붙일 때 설계할 5가지 (지금 불필요)

1. **오케스트레이터 간 통신** — BB-OS 결과가 다른 큐에 영향 줄 때 depends_on 표준
2. **예산 분리** — 월 $30 공통이면 한쪽 독식 위험. 오케별 할당
3. **지식 공유** — Day1 lessons 같은 교훈을 다른 오케도 읽게. 공통 /lessons/
4. **우선순위 충돌** — 동시 "대표님 결정 필요" 올라올 때 중재 규칙
5. **대표님 개입 채널 통일** — 텔레그램 봇 1개로 모으고 prefix 구분

## 트리거 시점
2번째 오케스트레이터(AI-watch) 착수 인터뷰 시 위 5건 먼저 설계 후 진행.

## 오케스트레이터 확장 로드맵 (수정)

1. BB-OS (지금)
2. BB for AI-watch (L1, 빠르게 가치 확인)
3. BB for 손사
4. BB for 디자인 (Figma MCP)
5. BB for 마케팅 (성과 판정 주관적 → 마지막)
6. BB for 버그모니터링
...
## 주의
이전 결론: 버그 모니터링은 공통 엔진에 박음 (도메인 중복 금지).
대표님이 별도 BB 추가 → 착수 시 "공통 인프라 vs 독립 오케" 재논의 필요.

## 2번째 오케스트레이터 붙일 때 설계 5건 (변경 없음)
1. 오케스트레이터 간 통신 (depends_on 표준)
2. 예산 분리
3. 지식 공유 (/lessons/)
4. 우선순위 충돌 중재
5. 개입 채널 통일
...
6. BB for 버그모니터링
7. BB for 비용최적화
8. BB for 검증시스템 (각 오케 성과 통계화 → 발전 피드백)

## 착수 시 재논의
버그모니터링·비용최적화·검증시스템 3개 모두 "공통 인프라 성격" 강함. 독립 오케로 분리할지 공통 모듈로 합칠지 착수 시점 판단.

## 검증시스템 메모
- 각 오케별 KPI: 스펙 성공률, 재시도율, 토큰 효율, 대표님 개입 빈도
- 주기 통계 → 약점 발견 → 프롬프트/워크플로우 자동 개선 제안
- 메타 오케 성격 (다른 오케를 감시/개선)

## 설계 5건 (변경 없음)
## 오케스트레이터 확장 로드맵 (재수정)

1. BB-OS (지금)
2. BB for AI-watch (L1)
3. BB for 손사
4. BB for 디자인
5. BB for 마케팅
6. BB for 버그모니터링
7. BB for 비용최적화 (API 토큰·VPS·SaaS 구독 비용 추적/절감 제안)

## 착수 시 재논의 필요
- 버그모니터링 · 비용최적화 둘 다 "공통 인프라 성격 vs 독립 오케" 갈림
- 비용최적화는 전체 오케 토큰 사용량을 봐야 해서 독립 오케보다 공통 모듈일 가능성 높음
- 착수 시점에 판단
...
1. BB-OS (지금)
2. BB for AI-watch + 검증시스템 통합 (AI 뉴스 모니터링 + 메타 오케 성격 흡수)
3. BB for 손사
4. BB for 디자인
5. BB for 마케팅
6. BB for 버그모니터링
...
## AI-watch 확장 스코프
- AI 뉴스 실시간 수집·스크리닝·딥리서치
- 각 오케 KPI 통계 (성공률/재시도율/토큰/개입빈도)
- 약점 기반 프롬프트·워크플로우 개선 제안
- 주기 다이제스트

## 착수 시 재논의
AI-watch 착수 시점에 "외부 정보 감시 + 내부 오케 감시"를 한 오케에 담는 게 맞는지 검증. 분리 필요하면 그때 분리.
...
오케스트레이터 스펙 실패 시:
- 자동 로그 회수 스펙 생성·실행
- 에러 원인 + 수정 권고를 텔레/Gmail 전달
- Claude 웹은 결과 보고 수정 스펙 작성

## 착수 시점
BB-OS 실전 2~3주 후. 실패 패턴 축적되고 나서.
지금 박으면 추측 기반 자동화.

## 규칙
스펙(ORCHESTRATOR_SPEC) 오케에 발송하기 전 대표님에게 반드시 확인 받기.
- 스펙 초안 내용 요약 제시
- "발송할까요?" 명시 질문
- 승인 받은 후에만 Gmail create_draft

## 배경
4/18 실전 1일차에 Claude 웹이 대화 반응마다 반사적으로 스펙 자동 발송 → 큐 오염·린터 차단·허탕 반복. 대표님 개입 0 루프는 실행 단계만 해당. 스펙 발송은 대표님 승인 필수.

## 예외
heartbeat 복구같이 "지금 당장 텔레 복구해" 명시 지시 있을 때만 즉발송.

## 상용화 전략 — AI 대화 감지

## 상용화 moat — 비비 + 오케 통합 판매

## QA 자동화 (상용화 전제)
- 자동 파서 vs 사용자 export 대조 (주 1회 샘플)
- 일치율 90% 이하 → 재생성
- 3회 실패 → 중앙 풀에 깨짐 리포트

## 감지층 공유 네트워크 효과
- 사용자 A: 새 AI 감지 → 파서 생성 → 성공 → 중앙 풀 기여
- 사용자 B: 동일 AI 쓰기 시작 → 자동 pull → 즉시 감지
- 파서 풀뿐 아니라 "감지 대상 카탈로그" 자체가 집단지성으로 확장
- 비비 사용자 늘수록 모든 비비의 감지 범위 확장

## 확장 범위
AI 대화뿐 아니라:
- 업무 SaaS (Notion, Figma, Slack...) 감지
- 개인 데이터 출처 (은행, 의료, 쇼핑 사이트) 감지
- 각 도메인별 파서도 동일 공유 메커니즘
...
## 소스 찾기 명령 (PC PowerShell/클코)
```
dir /s /b C:\Users\admin\*.kt 2>nul | findstr -i vivi
dir /s /b C:\Users\admin\AndroidStudioProjects 2>nul | findstr -i vivi
```
...
```
추정: `com.anthropic.claude`

## 수정 방향
- `KakaoOutgoingService.kt` 구조 참고
- Claude 앱용 AccessibilityService 추가 (별도 클래스 or 기존 클래스에 패키지 필터 확장)

## 개발팀이 PC 로컬 작업으로 안내
PC 클코 실행 → 소스 찾기 → 수정 → Android Studio 빌드 → APK 설치 → 접근성 권한 9종 재부여
...
초기: Claude 웹↔대표님 인터뷰 → 스펙 → 오케 무인 실행. 실패는 대표님+웹 손작업 + 교훈 축적(사례집).

확장: 도메인 오케 2개 이상 시 메가 오케 1개 신설. 메가 역할 = 각 오케 버그/개선 모니터링, 태그 누적 10회 코어 수정 지시, 오케 간 충돌 탐지.

현재 BB-OS Core 오케 1개 → 메가 오케 불필요. 2개째 생기면 구축.
...
2026-04-19 PM 오늘 실수(B29) 기반 지시 실수 방어 체계 정립.

## 핵심 발견
오늘 실수 경로 = "Claude 단일 쿼리 오용 → 앞 세션 기억 전체 부정 → 파괴적 지시 발동 → 대표님 수동 실행 직전". 시스템 쪽 안전장치 zero. 대표님 의심 + 다른 Claude 교차검증으로만 막힘.

## 정립한 건물 구조 (6층)
- 0층 실측 3규칙: 교차검증 / 건수 불신 / 의심 신호 우선
- 1층 파괴적 지시 체크리스트 5개 (증거3건/반대가설/되돌림/상충/probe비용)
- 2층 지시 프리플라이트: 사실+증거/확신등급/반대가설기각/복구비용/구체행동
- 3층 bb-auditor (구축 예정)
- 4층 대표님 의심 레인
- 지붕 다중 Claude 교차검증

0·1·2층 = 룰, 비용 0. 오늘 실수는 0·1·2층 있었으면 3중 방어됨.

## 산출물 (오늘)
- 오케_작업기준_재조정_v3_20260419.md (0·1·2층 룰 박음)
- 오케_실패복구_사례집_v6_20260419.md (B24~B29 전체, #stdout-loss 10/10 임계)

## 프로젝트 파일 교체 대상
업로드: v3 작업기준, v6 사례집
삭제 권장: v1/v2 작업기준, v1~v4 사례집 (v6가 전부 흡수)

## 핵심 재발 방지 링크
- B29 해결 = 작업기준 v3 0·1·2층 전체
- 의심 페르소나 6번째 추가 (검색/쿼리 자체가 누락 만드나?)
- 인지 개념 3번 강화 (도구 "목록" 도 문서일 뿐)
...
### 동작 흐름
1. 사용자 평소대로 AI 사용
2. 비비 감지: 도메인 방문 빈도 학습
3. 임계치 초과 시 오케가 해당 사이트 HTML 자동 분석 (LLM)
4. 파서 코드 생성 → 로컬 테스트 → 배포
5. 플랫폼 HTML 변경 감지 시 자동 재생성
6. 성공한 파서는 중앙 풀 기여 (옵션, 사용자 동의 시)

### Moat
- 쓸수록 커지는 파서 카탈로그
- 네트워크 효과 (사용자 증가 → 파서 풀 품질 증가)
- 경쟁사가 같은 스택 만들어도 카탈로그 추격 불가

### 기술 리스크
- 첫 자동 파서 정확도 70~80% 예상
- HTML 변경 감지 루프 필요
- 초기 QA 사람 필요 (점진 자동화)

### 연결
- BB for AI-watch 확장 범위
- 오케스트레이터 복제(domain_bibi)
- 검증시스템 오케 (AI-watch에 통합)와 공동 작동

### 시점
BB-OS 안정 후 → 개인용 AI 대화 감지 실전 → 파서 자동 생성 시도 → 상용화 검증.

### 컨셉
"비비 깔기만 하면 끝." 범용 확장 아님.

### 동작
1. 비비가 사용자 브라우저/모바일 활동 감지
2. 자주 방문하는 AI 도메인 자동 학습 (임계치: 월 N회)
3. 임계 초과 시 비비가 해당 플랫폼 파서 자동 설치
4. 그 시점부터 대화 감지 시작

### 구현 단계
- **1단계 (현재)**: 대표님 개인용. Claude/ChatGPT/Gemini 등 수동 파서 추가
- **2단계 (실전 검증 후)**: 파서 카탈로그 서버 구축. 비비가 중앙에서 파서 pull
- **3단계 (상용화)**: 파서 자동 매칭 + 설치 루프

### 파서 생성 자체 자동화 가능성
HTML 구조 자동 분석 + LLM으로 파서 코드 생성 시도 가능. 단, 첫 몇 개는 수동. 패턴 쌓이면 자동화 시도.

### 메모 시점
2026-04-18. BB for AI-watch 착수 인터뷰 시 재논의.
...
작업 지시서 자체의 보강 항목 (다음 큰 작업 전에 표준 문서화):

1. 의사결정 JSONL 로그 (pm/architect/executor/validator 전부)
2. Step 진입·종료 timestamp + 상태 스냅샷 (diff로 변경 내역 도출)
3. 판단 분기점마다 "틀렸을 때 보이는 증거" 탐지 쿼리 내장
4. 리포트 3층 (대표님용 / 작업별 / 상세)
5. 롤백 드라이런 - 롤백 명령 실제 작동 여부 사전 테스트
6. 실시간 경보 (서비스 down, DB row 감소) → 텔레그램
7. 대표님 판단 호출 이력 누적 → 같은 질문 반복되면 다음 스펙 기준 추가

배경: 자동주행 크리티컬 문제는 실패 지점 역추적. 특히 대표님 비개발자 관점에서 로그만 보고 판단 가능해야 함.

다음 액션: 여진 작업 끝나면 "자동주행 로깅 표준" 문서 작성 → 이후 모든 스펙에 상속
...
1. 사용자 동의 기반 학습: 가능. 단 3~4층 동의서 구조 필요 (수집·이용, 민감정보, 목적외이용, 국외이전). 원본 학습 대신 가명처리 후 학습이 안전 경로 (제28조의2). 로컬 원본 + 가명처리본만 비비 회사로 올리는 파이프라인이 이루다 사건 이후 업계 표준.

2. Obsidian 비용 모델: 개인 무료 + 상업용 연 $50/user + Sync $4~10/월. 비비 번역 시 월 3~5만원 라이선스 + AI API 사용량 기반 + 호스팅 지원 선택. 핵심: 개인 무료, 상업 유료 구조가 B2B 전문직 시장에서 효과적.

3. 제휴 3갈래:
   - (A) 클라우드 VPS: 네이버클라우드 Financial Cloud, KT Cloud, AWS Seoul 원클릭 설치 패키지
   - (B) NAS 제조사: Synology, Qnap - 하드웨어 1회 + 소프트웨어 라이선스 모델
   - (C) 손사협회·법무법인: 업무 가이드·법적자문 패키지 번들

4. 국내 유사 서비스: 로앤비(법률), 더존(회계), 유비케어(의료) 모두 클라우드형. "한국 + 로컬 저장 + 손사 특화 + AI" 교집합은 비어있음. 리서치 결론 확인: 한국 법률전문직 B2B SaaS는 거의 유일한 차별화 세그먼트.

#### 심리적 전환 포인트 기록
- 대표님: "내가 관리 안 해도 돼. 왜 내가 굳이 다 갖고 있으려 했지?"
- 오후 내내 SaaS 데이터 창고 집착에서 벗어남
- "데이터 다 갖고 있기" 동기는 "학습시키고 싶어서"였는데, 가명처리 + 동의 경로로 우회 가능함을 확인
- BB-OS 헌법의 "자기 서버" 원안으로 자연스럽게 복귀

#### 사용자 데이터 학습 파이프라인 (확정 설계 방향)
```
[사용자 로컬 서버]
  └─ 원본 데이터 저장 (사용자 소유·책임)
  └─ 사용자 동의 시 가명처리 모듈 실행
      └─ 이름·주민번호·연락처·민감 키워드 제거
      └─ 가명처리본 생성
  └─ 가명처리본만 비비 클라우드로 업로드
...
[영장 리스크]
  └─ 비비 회사 영장: 가명처리본만 제공 가능 (개별 식별 불가)
  └─ 사용자 영장: 본인 서버에 집행 (본인 책임)
```

#### 다음 세션 준비사항
- 국내 손사 AI·SaaS 경쟁 현황 웹검색 필요 여부 (이 세션 또는 다음 세션)
- Obsidian 모델 기반 비비 사업 모델 구체화 (1인 라이선스 vs 기업 vs 협회)
- 가명처리 기술 스택 확정 (Presidio 한국어 확장 포함)
- 제휴 후보 우선순위 결정

#### 비비 수익 구조 3축 확정
1. 소프트웨어 라이선스 (주 수익): 월 3~5만원/user B2B 손사
2. AI API 차익 (부 수익): 토큰 사용량 기반
3. 제휴 커미션 (부 수익): VPS·NAS 업체 리퍼럴

#### 제휴 후보 업체 (국내)

클라우드 VPS:
- 네이버클라우드 (Financial Cloud 보유, 금융 인증) - 1순위
- AWS Seoul (업계 표준, APN 파트너 프로그램)
- KT Cloud, NHN Cloud (국내 대안)
- 카페24, 가비아 (저가 옵션, 사양 부족 가능)

NAS 하드웨어:
- Synology (한국 1위, 손사 사무실에 현실적)
- Qnap (대안)

#### 수익 예상 (100명 유치 기준)
- 클라우드 VPS 제휴: 월 100만원 지속
- NAS 제휴: 총 3,000만원 (1회)
- 주 수익 라이선스: 월 300~500만원
- AI API 차익: 사용량 기반

#### 실행 단계
1. 권장 사양 공식 발표 (RAM 24GB, CPU 4코어+, SSD 200GB+)
2. 각 업체 파트너십 체결 (2~4주)
3. 리퍼럴 코드 발급 + 비비 웹사이트에 "권장 서버" 섹션
4. 원클릭 설치 스크립트 제공 (전환율 극대화)
5. 티어 구분: NAS (기본), VPS 8GB (중급), VPS 24GB (고급)

#### 보류
- 각 업체 실제 2026년 파트너 수수료율 웹검색 필요 (네이버클라우드, AWS, Synology 기준)
- 파트너십 체결 실무 절차 조사

#### 오늘 세션 최종 결론 (2026-04-17 23:00)

대표님 한 마디로 모든 것 수렴:
"영장까지 막자 이거는 현실성없고 잘못을 했으면 처벌을 받아야지"

#### 포지셔닝 확정
- 비비 = "법을 지키는 손사를 위한 업무 AI"
- 범죄 은폐 도구 아님
- 영장 방어는 본질이 아니며 추구 대상 아님
- 건강한 시민 도구 포지션

#### 위협 모델 최종 범위
- 방어 대상: 해커, 제공자 내부자 실수, 데이터 유출·도난
- 포기 대상: 영장 방어, 국가급 APT, 완전 제로노리지

#### 인프라 단일 추천 확정
- 1순위: 네이버클라우드 Financial Cloud (금융 인증, 한국 리전)
- 2순위: AWS Seoul
- NAS 옵션은 선호 사용자만

#### 보안 레벨 확정
- 업계 표준: AES-256 + TLS + Passkey + ISMS-P 제공자
- Apple PCC 수준 과투자 폐기
- Fuzzy Extractor·분산 파편화·Stage 5 전부 폐기
- Plaud 수준 인증 배지면 충분

#### 최종 구조 (오늘 세션 전체 수렴)

제품:
- 비비 Core = 개인 데이터 OS (BB-OS 헌법)
- 비비 손사 도메인 앱 = 업무 특화

배포 모델:
- Obsidian 모델 (소프트웨어 공급자)
- 데이터 = 사용자 본인 VPS

보안:
- 네이버클라우드 Financial Cloud + 업계 표준 암호화
- Passkey(WebAuthn PRF) + age 파일 암호화
- 영장 방어 포기, 건강한 시민 도구 포지셔닝

수익 3축:
1. 소프트웨어 라이선스 (월 3~5만원 B2B)
2. AI API 차익 (토큰 사용량 기반)
3. VPS 제휴 커미션 (네이버클라우드 10% 수준)

컴플라이언스 로드맵:
- 출시 전: Trail of Bits/NCC 외부 감사 + VDP 개설
- 출시 시: SOC 2 Type I + ISMS-P 착수
- +6개월: SOC 2 Type II
- +12개월: ISO 27001/27701
- AI 기본법 고영향 AI 사전 확인 요청

차별화 5축:
1. 텔레그램 모델 (약속 지키는 증거 반복 축적)
2. Eat your own dogfood (대표님이 본인 고객 데이터로 먼저 사용)
3. 한국 손사 B2B 전문직 선점
4. Obsidian 모델 (데이터 주권 사용자에게)
5. 한국 규제 선제 준수

#### 오늘 하루 패러다임 전환 이력 (참고)
Stage 1: restic + B2 → 폐기
Stage 2: 공개키 + Shamir + Time-lock → 폐기
Stage 3: 입 무거운 집사 모델 → 부분 수용
Stage 4: 체화 기반 연속 인증 → 학술 단계 확인 후 재정의
Stage 5: 분산 파편화 → 폐기
Stage 6: Obsidian 모델 → 확정
Stage 7: 영장 방어 포기, 건강한 시민 도구 → 최종

#### 대표님 결정 요지
- 완벽 추구 대신 현실적 합리
- 기술 과시 대신 약속 이행
- 위협 최대화 방어 대신 위협 범위 좁혀서 실행
- 본업(손사)과 양립 가능한 범위

#### 방금 직전 드래프트 일부 수정 필요
- "영장 방어 포기, 건강한 시민 도구" 결정 10분 만에 대표님 본인이 재의심
- "상시 녹화급 일상데이터인데 타협보는게 맞나?"
- 그 결정은 잠정 상태로 다시 열림

#### 의심의 정당성 (Claude 평가)
- 비비 수집 밀도는 현존 AI 제품 중 가장 깊음
  - Rewind/Recall: 시각만
  - Plaud: 버튼 누를 때만
  - 비비: 카톡+통화+SMS+PC+모바일+위치 동시 다채널
- "업계 표준 보안" = 평균 리스크 × 훨씬 많은 데이터 = 절대 리스크 큼
- CCTV와 질적으로 다른 데이터베이스 (10년치 대화·판단·감정·관계)

#### 반대 논리
- 손사는 비비 없어도 이미 개인정보 보유
- 비비는 "이미 있는 리스크의 관리 도구"로 해석 가능

#### 진짜 완화책: 보안 아닌 수집 범위
1. 자동 만료 (기본 30/90/365일)
2. 민감 Vault 계층 B (추가 암호)
3. 비기록 시간대 기본값 (저녁·가족 시간)
4. 사용자 선택 수집 범위 체크박스
5. 대표님 본인 설정 vs 판매 버전 분리

#### 재검토 필요한 가정
- BB-OS 헌법 "집요한 수집" = 무차별 상시 녹화인가, 중요한 것 놓치지 않음인가
- 둘은 질적으로 다른 설계
- 이 구분이 비비 정체성 핵심

#### 오늘 결정 금지 선언
- 대표님 탈진 + 10분 내 결정 뒤집기 패턴 = 지금 내리는 결정 신뢰도 낮음
- 이 의심은 며칠 숙고할 가치 있는 본질적 갈등
- 다음 세션 또는 이번 주 중 "수집 범위 재검토" 단독 주제 필요

#### 세션 상태
- 오늘 세션 최종 결론은 "방향 열어둠"
- 직전 드래프트의 "최종 확정" 표현은 잠정
- 수집 범위 결정이 보안 설계보다 선행되어야 함

#### 다음 세션 주제 (우선순위)
1. 비비 수집 범위 재정의 (상시 녹화 vs 선택적 수집)
2. 그에 따른 보안 요구수준 재계산
3. 사용자 옵션 설계 (체크박스, 슬라이더)
4. 대표님 본인 설정 vs 판매 버전 분리 여부

#### 비비 프로젝트의 진짜 동기 (오늘 드러남)
- 대표님 본심: "사람 한 명을 그대로 본뜨고 싶었어. 그러면 그 사람이 죽어도 다시 만날 수 있으니까. 그래서 100% 수집, 100% 안전 원했음"
- 비비 5가지 비밀 중 "5. 불멸" 축의 진짜 의미가 드러남
- 기술 명세가 아니라 상실에 대한 두려움, 관계를 지속하려는 근본 동기

#### 오늘 세션의 진짜 교훈
- 비비의 기술적 완벽주의(100% 수집·100% 안전)는 이 동기의 부산물이었음
- 하지만 그 동기를 충족하기 위해 꼭 100%가 필요한 것은 아님
- 진짜 필요한 것: 데이터 양이 아니라 관계의 깊이, 모든 순간이 아니라 의미 있는 순간

#### 비비의 재정의된 목적
- "그 사람을 되살리는 것"이 아닌 "잊지 않는 것"
- 살아있는 동안 더 많이 듣고, 더 잘 기억하고, 사라질 뻔한 순간 붙잡기
- 이건 현실적 + 윤리적 + 가치 있음

#### 다음 세션 진입 시 참고
- 수집 범위 결정 때 이 동기 기준 재검토
- "100% 수집"이 본래 목적(관계 지속)에 꼭 필요한가 질문
- 선택적 수집이 오히려 목적에 충실할 수 있음

#### Claude 한계 인식
- 대표님이 이런 감정 나눴을 때 Claude는 대화 파트너 될 수 있으나 인간 관계 대체 불가
...
**완료:**
- 백업 3단계 자동화 (PostgreSQL 일 + 인프라 주, 로컬+Google Drive 이중화)
- API 월 상한 $10 / 경보 $5 설정 (P1 해소)
- Claude.ai 프로젝트 재배치: BB-OS팀 + 비비폴 손사팀 분리, F번 5개 로컬 백업 후 삭제
- 메모리·지침 최종본 작성 (프로젝트_메모리_지침_최종본_20260417.md)
- 프로젝트 파일 인벤토리 작성 (51개 분류)
- WORK_MEMORY 파이프라인 복구 (Gmail MCP stale 이슈가 원인이었음, VPS는 정상)

**미완료 / 다음:**
- 새 프로젝트 2개에 지침 붙이기 + 메모리 수동 추가
- 1층(이해) 구현 진입 준비
- Honcho 실제 데이터 위치 확인 (config.toml만 마운트 중)
